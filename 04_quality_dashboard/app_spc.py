...
# ═══════════════════════════════════════════════════════════════════════════
#  PAGE 6 — SPC MONITORING
# ═══════════════════════════════════════════════════════════════════════════

elif page == "SPC Monitoring":
    st.markdown("## 📈 SPC Monitoring")
    st.markdown("Statistical Process Control for critical dimensional and GD&T features")
    st.markdown("---")

    if spc.empty:
        st.warning("No SPC measurement records for selected date range.")
    else:
        feature_options = sorted(spc["feature_name"].dropna().unique().tolist())
        ...
        out_of_spec_rate = feature_df["out_of_spec"].mean() * 100

        control_violations = subgroup[(subgroup["xbar"] > ucl) | (subgroup["xbar"] < lcl)]

        # Simple drift signal: compare recent subgroup average to earlier average.
        recent_n = min(15, len(subgroup) // 3 if len(subgroup) >= 9 else len(subgroup))
        if recent_n > 0:
            early_mean = subgroup.head(recent_n)["xbar"].mean()
            recent_mean = subgroup.tail(recent_n)["xbar"].mean()
            drift = recent_mean - early_mean
        else:
            drift = 0

        capability_status = "Capable" if cpk >= 1.33 else ("Marginal" if cpk >= 1.0 else "Not Capable")
        control_status = "Stable" if len(control_violations) == 0 else "Investigate"
        drift_status = "Drifting Up" if drift > sigma else ("Drifting Down" if drift < -sigma else "No Major Drift")

        c1, c2, c3, c4, c5 = st.columns(5)
        c1.markdown(kpi_card("Cp", f"{cp:.2f}", "green" if cp >= 1.33 else ("amber" if cp >= 1.0 else "red"), "Potential capability"), unsafe_allow_html=True)
        c2.markdown(kpi_card("Cpk", f"{cpk:.2f}", "green" if cpk >= 1.33 else ("amber" if cpk >= 1.0 else "red"), capability_status), unsafe_allow_html=True)
        c3.markdown(kpi_card("Control Violations", str(len(control_violations)), "green" if len(control_violations) == 0 else "red", control_status), unsafe_allow_html=True)
        c4.markdown(kpi_card("Out-of-Spec Rate", f"{out_of_spec_rate:.2f}%", "green" if out_of_spec_rate == 0 else ("amber" if out_of_spec_rate < 2 else "red"), "Actual failures"), unsafe_allow_html=True)
        c5.markdown(kpi_card("Drift Signal", drift_status, "green" if drift_status == "No Major Drift" else "amber", f"Δ {drift:.4f} {unit}"), unsafe_allow_html=True)

        st.markdown("<br>", unsafe_allow_html=True)

        if capability_status == "Not Capable":
            st.markdown('<div class="insight-box red">🔴 <b>Capability Risk:</b> Process Cpk is below 1.00. The process is not consistently capable of meeting specification limits.</div>', unsafe_allow_html=True)
        elif capability_status == "Marginal":
            st.markdown('<div class="insight-box amber">🟡 <b>Capability Warning:</b> Process Cpk is between 1.00 and 1.33. Monitor closely and consider process improvement.</div>', unsafe_allow_html=True)
        else:
            st.markdown('<div class="insight-box green">🟢 <b>Capability Good:</b> Process Cpk is at or above 1.33, indicating acceptable process capability.</div>', unsafe_allow_html=True)

        col1, col2 = st.columns(2)
        with col1:
            st.markdown('<div class="section-header">X-Bar Control Chart</div>', unsafe_allow_html=True)
            fig = go.Figure()
            fig.add_trace(go.Scatter(
                x=subgroup["subgroup_id"], y=subgroup["xbar"],
                mode="lines+markers", name="Subgroup Mean", line=dict(color=ACCENT, width=2)
            ))
            ...
            fig.add_hline(y=usl, line_dash="dot", line_color=AMBER, annotation_text="USL")
            fig.add_hline(y=lsl, line_dash="dot", line_color=AMBER, annotation_text="LSL")
            fig.update_xaxes(title="Subgroup")
            fig.update_yaxes(title=f"Measured Value ({unit})")
            st.plotly_chart(style_fig(fig, height=420), use_container_width=True)

        with col2:
            st.markdown('<div class="section-header">Subgroup Range Chart</div>', unsafe_allow_html=True)
            rbar = subgroup["subgroup_range"].mean()
            fig2 = go.Figure()
            fig2.add_trace(go.Scatter(
                x=subgroup["subgroup_id"], y=subgroup["subgroup_range"],
                mode="lines+markers", name="Range", line=dict(color=AMBER, width=2)
            ))
            fig2.add_hline(y=rbar, line_dash="solid", line_color=GREEN, annotation_text="R-bar")
        ...
        col3, col4 = st.columns(2)
        with col3:
            st.markdown('<div class="section-header">Measurement Distribution vs Specification</div>', unsafe_allow_html=True)
            fig3 = px.histogram(feature_df, x="measured_value", nbins=35, color_discrete_sequence=[ACCENT])
            fig3.add_vline(x=lsl, line_dash="dash", line_color=RED, annotation_text="LSL")
            fig3.add_vline(x=usl, line_dash="dash", line_color=RED, annotation_text="USL")
            fig3.add_vline(x=mean, line_dash="solid", line_color=GREEN, annotation_text="Mean")
            fig3.update_xaxes(title=f"Measured Value ({unit})")
            fig3.update_yaxes(title="Count")
            st.plotly_chart(style_fig(fig3, height=340), use_container_width=True)

        with col4:
            st.markdown('<div class="section-header">SPC Summary</div>', unsafe_allow_html=True)
            summary = pd.DataFrame([
                ...
                ["Cp", f"{cp:.2f}"],
                ["Cpk", f"{cpk:.2f}"],
                ["Process Status", capability_status],
                ["Control Status", control_status],
            ], columns=["Metric", "Value"])
            st.dataframe(summary, use_container_width=True, hide_index=True)

        st.markdown('<div class="section-header">Capability by Critical Feature</div>', unsafe_allow_html=True)
        capability_rows = []
        for fname, group in spc.groupby("feature_name"):
            vals = group["measured_value"].dropna()
            sig = vals.std(ddof=1)
            avg = vals.mean()
            group_lsl = float(group["lsl"].iloc[0])
            group_usl = float(group["usl"].iloc[0])
            group_cp = (group_usl - group_lsl) / (6 * sig) if sig > 0 else np.nan
            group_cpk = min((group_usl - avg) / (3 * sig), (avg - group_lsl) / (3 * sig)) if sig > 0 else np.nan
            capability_rows.append({
                ...
                "Cp": group_cp,
                "Cpk": group_cpk,
                "Out-of-Spec %": group["out_of_spec"].mean() * 100,
                "Status": "Capable" if group_cpk >= 1.33 else ("Marginal" if group_cpk >= 1.0 else "Not Capable"),
            })
        cap_df = pd.DataFrame(capability_rows).sort_values("Cpk")
        st.dataframe(
